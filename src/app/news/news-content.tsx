    ...
    <main className="bg-[#f5f5f7] pb-24 text-[#1d1d1f]">
      <section className="border-b border-blue-100 bg-white">
        <div className="mx-auto max-w-7xl px-5 py-10 md:py-14">
          <div className="flex flex-col gap-7 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-sm font-semibold tracking-[0.12em] text-blue-600">CHANMENG NEWS</p>
            ...
            <form onSubmit={submitSearch} role="search" className="flex w-full max-w-2xl items-center rounded-full border-2 border-blue-500 bg-white p-1.5 shadow-[0_8px_30px_rgba(37,99,235,.10)]">
              ...
              <button type="submit" className="rounded-full bg-gradient-to-r from-blue-500 to-blue-600 px-7 py-2.5 text-sm font-semibold text-white shadow-sm transition-transform hover:scale-[1.02] active:scale-[0.97] focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2">搜索</button>
            </form>
          </div>
        </div>
      </section>
      ...
      <div className="sticky top-12 z-30 border-b border-black/[0.05] bg-white/92 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl gap-2 overflow-x-auto px-5 py-3">
          {CATEGORIES.map((item) => (
            <button key={item} type="button" onClick={() => setCategory(item)} className={`whitespace-nowrap rounded-full px-4 py-2 text-sm transition-colors ${category === item ? 'bg-blue-600 font-medium text-white' : 'text-gray-600 hover:bg-blue-50 hover:text-blue-600'}`}>
              {item}
            </button>
          ))}
        </div>
      ...
      {true ? (
        <section id="news-results" className="mx-auto max-w-7xl px-5 pt-10">
          <div className="mb-7 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-sm text-blue-600">{query ? `关键词：${query}` : category === '全部' ? '全部内容' : `分类：${category}`}</p>
              <h2 className="mt-2 text-3xl font-semibold tracking-[-0.04em]">{query ? '新闻搜索结果' : '新闻'}</h2>
            </div>
            {query ? <button type="button" onClick={() => { setSearchText(''); setQuery('') }} className="self-start rounded-full bg-white px-4 py-2 text-sm text-gray-600 shadow-sm ring-1 ring-black/[0.06] hover:text-blue-600">清除搜索</button> : <span className="text-sm text-gray-400">共 {articles.length} 篇内容</span>}
          </div>
          <div className="overflow-hidden rounded-[24px] bg-white px-5 shadow-sm ring-1 ring-black/[0.05] md:px-8">
            ...
            {articles.length ? articles.map((article) => (
              <Link key={article.slug} href={`/news/${article.slug}`} className="group grid gap-2 border-b border-gray-200 py-5 last:border-0 md:grid-cols-[210px_210px_minmax(0,1fr)] md:items-center">
                ...
                <span className="text-base font-medium text-[#1d1d1f] group-hover:text-blue-600 group-hover:underline md:text-[17px]">{article.title}</span>
              </Link>
            )) : (
      ...
      <section id="news-results" className="mx-auto max-w-7xl px-5 pt-7">
        {featured ? (
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
            <Link href={`/news/${featured.slug}`} className="group relative min-h-[430px] overflow-hidden rounded-[28px] bg-gradient-to-br from-blue-500 via-blue-600 to-[#1749c9] p-8 text-white shadow-[0_18px_55px_rgba(37,99,235,.18)] md:p-12">
              ...
              <div className="absolute bottom-[-100px] right-[18%] h-64 w-64 rounded-full bg-sky-300/20 blur-2xl" />
              <div className="relative flex h-full max-w-3xl flex-col justify-between">
                ...
                <div>
                  ...
                  <span className="mt-7 inline-block rounded-full bg-white px-5 py-2.5 text-sm font-medium text-blue-600">阅读全文</span>
                </div>
              </div>
            </Link>
            ...
            <aside className="rounded-[28px] bg-white p-6 shadow-sm ring-1 ring-black/[0.04]">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-blue-600">实时推荐</span>
              </div>
              <div className="mt-5 divide-y divide-gray-100">
                {hot.map((article, index) => (
                  <Link key={article.slug} href={`/news/${article.slug}`} className="group flex gap-4 py-4 first:pt-2">
                    <span className={`text-2xl font-semibold ${index < 3 ? 'text-blue-600' : 'text-gray-300'}`}>{String(index + 1).padStart(2, '0')}</span>
                    <div className="min-w-0"><p className="line-clamp-2 text-[15px] font-medium leading-6 group-hover:text-blue-600">{article.title}</p><p className="mt-1 text-xs text-gray-400">{article.category} · {article.date}</p></div>
                  </Link>
                ))}
              </div>
            </aside>
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {rest.map((article, index) => (
              <Link key={article.slug} href={`/news/${article.slug}`} className="group flex min-h-[310px] flex-col justify-between overflow-hidden rounded-[24px] bg-white p-7 shadow-sm ring-1 ring-black/[0.04] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_18px_45px_rgba(0,0,0,.09)]">
                <div>
                  <div className="flex items-center justify-between"><span className="rounded-full bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-600">{article.category}</span><span className="text-xs text-gray-400">{article.date}</span></div>
                  <h3 className="mt-7 text-2xl font-semibold leading-tight tracking-[-0.035em] group-hover:text-blue-600">{article.title}</h3>
                  <p className="mt-4 line-clamp-3 text-sm leading-7 text-gray-500">{article.summary}</p>
                </div>
                <div className="mt-7 flex items-center justify-between border-t border-gray-100 pt-5"><span className="text-sm font-medium text-blue-600">阅读全文</span><span className="text-xs text-gray-300">NO.{String(index + 2).padStart(2, '0')}</span></div>
              </Link>
            ))}
          </div>
        </section>
